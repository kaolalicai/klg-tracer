...
describe('http client with koa hook test', async function () {
  ...
  beforeAll((done) => {
    ...
    app.use(async (ctx, next) => {
      ...
      await request.get('http://myapp.com').query({msg: 'hello'})
        .set('Accept', 'application/json').then()
      await request.post('http://myapp.com').send({msg: 'hello'}).then()
      await next()
    })
    server = http.createServer(app.callback()).listen(4015)
    done()
  })
  ...
  it(' test query ', async () => {
    ...
    const res = await request.get('http://localhost:4015/hello').query({msg: 'hello', userId: '123123123'})
